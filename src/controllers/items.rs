...
pub async fn create(
    ...
    let item = items::Model::create(&ctx.db, create).await?;
...
async fn read(
    ...
    format::json(items::Model::find_by_id(&ctx.db, id).await?.into())
}
...
pub async fn update(
    ...
    format::json(items::Model::update(&ctx.db, id, update).await?.into())
}
